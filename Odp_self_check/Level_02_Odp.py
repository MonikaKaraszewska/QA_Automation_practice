from selenium import webdriver
from selenium.webdriver import Keys
from selenium.webdriver.common.by import By
import time


class Test_Helion:
    driver=''

    def setup_method(self):
        self.driver = webdriver.Chrome()
        self.driver.implicitly_wait(10)
        self.driver.get("https://helion.pl")
        expectedTytul = "Księgarnia informatyczna helion.pl - informatyka w najlepszym wydaniu."
        tytul = self.driver.title
        assert expectedTytul == tytul, f'miałobyc {expectedTytul} A JEST - "{tytul}"'


    def test_1(self):
        searchbar = self.driver.find_element(By.ID, 'inputSearch')
        searchbar.send_keys('devops', Keys.ENTER)

        expected_Path = 'https://helion.pl/search/?qa=&szukaj=devops'
        actual_path = self.driver.current_url

        assert expected_Path == actual_path, f"error path expected path{expected_Path} a mamy {actual_path}"

        search_book1 = self.driver.find_element(By.CLASS_NAME, 'lazy')
        search_book1.click()
        tytul2 = self.driver.title
        tytul_ok = "DevOps w praktyce. Wdrażanie narzędzi Terraform, Azure DevOps, Kubernetes i Jenkins. Wydanie II Mikael Krief. Książka, ebook - Księgarnia informatyczna Helion"
        assert tytul2 == tytul_ok, f'cos jest nie tak'

        search_koszyk_button = self.driver.find_element(By.ID, "addToBasket_devpr2")
        search_koszyk_button.click()


    def test_2(self):
        searchbar = self.driver.find_element(By.ID, 'inputSearch')
        searchbar.send_keys('testing', Keys.ENTER)

        expected_Path = 'https://helion.pl/search/?qa=&szukaj=testing'
        actual_path = self.driver.current_url

        assert expected_Path == actual_path, f"error path expected path{expected_Path} a mamy {actual_path}"

        search_book1 = self.driver.find_element(By.CLASS_NAME, 'lazy')
        search_book1.click()
        tytul2 = self.driver.title
        print(tytul2)
        tytul_ok = "Testowanie kodu z React Testing Library. Jak tworzyć testy, które będą proste w utrzymaniu i modyfikacji Scottie Crump. Książka, ebook - Księgarnia informatyczna Helion"
        assert tytul2 == tytul_ok, f'cos jest nie tak'

        search_koszyk_button = self.driver.find_element(By.ID, "addToBasket_bibrea")
        search_koszyk_button.click()

    def teardown_method(self):
        time.sleep(5)
        self.driver.quit()

